...
// --- Impor Ikon ---
import {
  Package,        // Untuk Detail Paket
  FileText,       // Untuk Narasi/Teks Soal
  List,           // Untuk Opsi
  Key,            // Untuk Kunci Jawaban
  BookOpen,       // Untuk Pembahasan
  AlertTriangle,  // Untuk Error
  CheckCircle2    // Untuk Sukses
} from 'lucide-react';
...
export default function AdminTambahSoal() {
  ...
  // --- 🌟 EFEK INTERAKTIF 🌟 ---
  // Ini akan mengubah placeholder Kunci & Opsi secara dinamis
  useEffect(() => {
    switch (tipeSoal) {
      case 'pg':
      ...
      case 'isian':
        ...
        setKunciJawaban(CONTOH_KUNCI_PG); // Kunci isian sama dgn PG
        break;
      default:
        setOpsiJawaban(CONTOH_OPSI_PG);
        setKunciJawaban(CONTOH_KUNCI_PG);
    }
  }, [tipeSoal]); // <-- Dijalankan setiap 'tipeSoal' berubah
  ...
  useEffect(() => {
    if (subtesId === 'tkp') {
      ...
      // Jika bukan TKP, kembali ke settingan 'tipeSoal'
      if (tipeSoal === 'pg') setKunciJawaban(CONTOH_KUNCI_PG);
    }
  }, [subtesId, tipeSoal]); // Dijalankan saat subtes atau tipe berubah

  // --- Logika HandleSubmit (Dimodifikasi untuk Modal) ---
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Kosongkan form untuk soal berikutnya
  ...
  return (
    <>
      ...
      {/* CSS untuk .input-field (sudah ada di ManajemenPaket, tapi kita tambahkan di sini untuk jaga-jaga) */}
      <style>{`
